...
@Component(name = "Validate Locked Pages -Component", service = WorkflowProcess.class, immediate = true, property = {
		"process.label = Validate Locked Pages Process" }, configurationPid = "com.tda.apac.core.workflows.ValidateLockedPages")

public class ValidateLockedPages implements WorkflowProcess {
		...
		try {
			...
			if (pageManager != null) {
				...
				if (page != null) {
					...
					if (isPageLocked) {
						workItem.getWorkflowData().getMetaDataMap().put("ispagelocked", true);
					} else {
						workItem.getWorkflowData().getMetaDataMap().put("ispagelocked", false);
						page.lock();
						log.info(":::::page is locked");
					}
				} else {
